...
class MultivariateGaussianDistribution(BaseDistribution, metaclass=abc.ABCMeta):

    def __init__(self, n: int, dtype: torch.dtype = torch.float32, **kargs):
        """
        ...
        self.mean = nn.Parameter(
            torch.zeros(self.n, dtype=self.dtype),
            requires_grad=False)  # type: torch.nn.Parameter

        self.logvars = nn.Parameter(
            torch.ones(self.n, dtype=dtype) * np.log(2. / (self.n)),
            requires_grad=False)
