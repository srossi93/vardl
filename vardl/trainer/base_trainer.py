...
class BaseTrainer(abc.ABC):
    ...
    def _train_batch(self, data, target, train_log_interval=1000):
        ...
        self.tb_logger.scalar_summary('loss/train/nll',
                                      self.compute_nell(output, target, len(self.train_dataloader.dataset),
        ...
        if type(self.model.likelihood) is Gaussian:
            self.tb_logger.scalar_summary('model/noise', self.model.likelihood.log_noise_var, self.current_iteration)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for data, target in self.test_dataloader:
                ...
                output = self.model(data)
                batch_nell = self.compute_nell(output, target, 1, 1)
        ...
        if self.test_verbose:
            logger.info('Test || iter=%5d   mnll=%01.03e   error=%8.3f   (%.2f ms per batch)' %
                        (self.current_iteration, test_nell.item(), test_error.item(),
                         (t1-t0)/len(self.test_dataloader) * 1000))
